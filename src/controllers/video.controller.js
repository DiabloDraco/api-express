...
export let GETVIDEO = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .send({ status: 500, message: "Error", reason: error.message });
  }
};
...
export let POSTVIDEO = (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send({ status: 200, name: newFileName, old: oldPath });
  } catch (error) {
    return res
      .status(500)
      .send({ status: 500, message: "Error", reason: error.message });
...
export let DELETEVIDEO = (req, res) => {
  try {
    ...
    return res.status(200).json({
      status: 200,
      message: "Video " + videoName + " Deleted successful",
    });
  } catch (error) {
    return res
      .status(500)
      .json({ status: 500, message: "Error", reason: error.message });
  }
};
...
export let GETVIDEONAMES = (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 200, files: files, fileCount: files.length });
  } catch (error) {
...
export let TEST = (req, res) => {
  try {
    res.status(200).json({ status: 200, work: req.body });
  } catch (error) {
    res.status(400).json({ status: 400, message: error.message });
  }
};
